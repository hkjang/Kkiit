...
function AuthAdmin() {
  const { notify } = useApp(); const [items, setItems] = useState<AuthProvider[]>([]); const [dialogOpen, setDialogOpen] = useState(false); const [form, setForm] = useState<ProviderForm>(initialProvider); const [oauthSetting, setOauthSetting] = useState<Setting | null>(null); const [callbackBase, setCallbackBase] = useState('')
  const load = () => Promise.all([api<{ items: AuthProvider[] }>('/api/v1/admin/auth-providers').then((data) => setItems(data.items)), api<{ items: Setting[] }>('/api/v1/admin/settings').then((data) => { const setting = data.items.find((item) => item.key === 'auth.oauth') ?? null; setOauthSetting(setting); const object = setting?.value as Record<string, unknown> | undefined; setCallbackBase(typeof object?.callback_base_url === 'string' ? object.callback_base_url : '') })]).catch((error) => notify(error.message, 'error'))
  useEffect(() => { void load() }, [])
...
// Display only: keep the server's policy matching and the form's save format.
function ApprovalConditions({ conditions }: { conditions: unknown }) {
  const lines: string[] = []
  let needsReview = false
  if (!conditions || typeof conditions !== 'object' || Array.isArray(conditions)) {
    needsReview = true
  } else {
    const order = ['min_amount', 'max_amount', 'service_types', 'seller_levels', 'quality_score_below']
    for (const [key, value] of Object.entries(conditions).sort(([a], [b]) => order.indexOf(a) - order.indexOf(b))) {
      if (key === 'min_amount' || key === 'max_amount' || key === 'quality_score_below') {
        if (typeof value !== 'number' || !Number.isFinite(value) || value < 0) {
          needsReview = true
          continue
        }
        if (key === 'quality_score_below') {
          lines.push(`품질 점수 ${value} 미만 (미산정 상품 포함)`)
        } else {
          const amount = value.toLocaleString('ko-KR', { maximumFractionDigits: 20 })
          lines.push(key === 'min_amount' ? `최소 금액 ${amount}원 이상` : `최대 금액 ${amount}원 이하`)
        }
      } else if (key === 'service_types' || key === 'seller_levels') {
        if (!Array.isArray(value) || !value.every((item) => typeof item === 'string' && item.trim().length > 0)) {
          needsReview = true
          continue
        }
        if (value.length > 0) lines.push(`${key === 'service_types' ? '서비스 유형' : '판매자 등급'} ${value.join(', ')}`)
      } else {
        needsReview = true
      }
    }
  }
  if (needsReview) lines.push('일부 조건 확인 필요')
  if (lines.length === 0) lines.push('조건 없음 · 모든 상품에 적용')
  return <Box sx={{ mt: 1, overflowWrap: 'anywhere' }}>{lines.map((line, index) => <Typography key={index} variant="body2" color="text.secondary">{line}</Typography>)}</Box>
}
...
function ApprovalAdmin() {
  ...
  const toggle = async (policy: ApprovalPolicy) => { try { await api(`/api/v1/admin/approvals/policies/${policy.id}`, { method: 'PUT', body: JSON.stringify({ resource_type: policy.resource_type, name: policy.name, enabled: !policy.enabled, priority: policy.priority, conditions: policy.conditions, steps: policy.steps }) }); load(); notify(!policy.enabled ? '승인 절차를 활성화했습니다.' : '승인 절차를 비활성화했습니다. 이후 요청은 절차를 건너뜁니다.', 'success') } catch (e) { notify(e instanceof Error ? e.message : '저장 실패', 'error') } }
